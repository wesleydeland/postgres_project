-- Query 4
-- Scenario:
-- Answer: What are the top skills based on salary?
-- • Look at the average salary associated with each skill for Software Engineer positions
-- • Focuses on roles with specified salaries, regardless of location
-- • Why? It reveals how different skills impact salary levels for Software Engineers and helps identify
-- the most financially rewarding skills to acquire or improve

SELECT 
    skills,
    ROUND(AVG(jobs.salary_year_avg)) AS avg_salary
FROM job_postings_fact AS jobs
INNER JOIN skills_job_dim ON jobs.job_id = skills_job_dim.job_id
INNER JOIN skills_dim ON skills_job_dim.skill_id = skills_dim.skill_id
WHERE jobs.job_title_short LIKE 'Software%Engineer%' AND 
      jobs.salary_year_avg IS NOT NULL
GROUP BY skills
ORDER BY avg_salary DESC
LIMIT 25;